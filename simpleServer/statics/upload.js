...
var uploader = new plupload.Uploader({
	...
	init: {
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = 'uploaded, share link: <br>https://historyofsalvation.oss-cn-hongkong.aliyuncs.com/' + get_uploaded_object_name(file.name);
            }
            else
            {
