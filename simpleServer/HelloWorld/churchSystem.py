...
def create_database_link():
    ...
    global conn, cursor
    conn, cursor = opendb('database4churchSystem.sqlite')
...
def grace_calculator(request):
    ...
    return render(request, 'churchSystem.html', my_dict)
...
def grace_calculator_viewer(request):
    ...
    return render(request, 'churchSystem_viewer.html', my_dict)
...
def accept_grace(request):
    ...
    return HttpResponseRedirect('/churchSystem')

def justViewer4churchSystem(request):
    my_dict = grace_calculator_data_all()
    #my_dict = grace_calculator_data() #This one is enough #they are a little difference, one shows no amounts. 
    return render(request, 'justViewer4churchSystem.html', my_dict)
