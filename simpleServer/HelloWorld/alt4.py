...
def alt4(request):
    ...
    if os.path.exists('language_voice_diction_hebrew/4web_restudy/已复习') and not os.path.exists('language_voice_diction_hebrew/4web_restudy/common_info'):
        my_dict_alt4['showinfo'] = '本日已复习'
        my_dict_alt4['con'] = 't1'
        my_dict_alt4['env'] = 't2'
        my_dict_alt4['ext'] = 't3'
        return HttpResponseRedirect('/alt1234') #复习完的直接回主页
        #return render(request, 'alt4.html', my_dict_alt4)
        ...
        if all_restudy_list[2] + all_restudy_list[1] + all_restudy_list[0]== 0:
            my_dict_alt4['showinfo'] = '待复习%s条(%s.%s)，将load继续' % (all_restudy, all_time_m, all_time_s)
        elif all_restudy_list[1] + all_restudy_list[0]== 0:
            my_dict_alt4['showinfo'] = '待复习%s(w:%s)条(%s.%s)，将load继续' % (all_restudy, all_restudy_list[2], all_time_m, all_time_s)
        elif all_restudy_list[0] == 0:
            my_dict_alt4['showinfo'] = '待复习%s(m:%s,w:%s)条(%s.%s)，将load继续' % (all_restudy, all_restudy_list[1], all_restudy_list[2], all_time_m, all_time_s)
        else:
            my_dict_alt4['showinfo'] = '待复习%s(y:%s,m:%s,w:%s)条(%s.%s)，将load继续' % (all_restudy, all_restudy_list[0], all_restudy_list[1], all_restudy_list[2], all_time_m, all_time_s)
    else:
        ...
        my_dict_alt4['showinfo'] = '将开始dump新的%s条复习，时间:%s' % (all_restudy, str(day))
        
#2、每次调用，读取一次dict，并相应accept_cmd修改数据库和dict
    if every_year_info:
        temp_info = list(every_year_info)
        for e_info in temp_info:
            my_dict_alt4['con'] = e_info[2]
            my_dict_alt4['env'] = e_info[3]
            my_dict_alt4['ext'] = e_info[4]
            my_dict_alt4['every_info'] = 'year'
            break
        ...
        for e_info in temp_info:
            ...
            my_dict_alt4['env'] = e_info[3]
            my_dict_alt4['ext'] = e_info[4]
            my_dict_alt4['every_info'] = 'common'
            break
#3、直到，查完事，结束并删除dump文件，生成已复习
    ...
    #处理下alt4的，算了，不处理旧的了，直接扔了，不要了，换新的
    return render(request, 'alt4.html', my_dict_alt4)
...
def accept_cmd_alt4(request):
    ...
    if every == 'year':
        if cmd == '8':
            every_year.delete('Con',my_dict_alt4['con'])
            every_month.add(Day=getnowtime('d'), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            common.add(Ymd=getdaystime(1), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            common.add(Ymd=getdaystime(5), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
        elif cmd == '2':
            every_year.delete('Con',my_dict_alt4['con'])
            common.add(Ymd=getdaystime(30), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
        every_year_info.pop(0)
        dump2file('language_voice_diction_hebrew/4web_restudy/every_year_info', every_year_info)
    ############处理月的##############
    elif every == 'month':
        if cmd == '8':
            every_month.delete('Con', my_dict_alt4['con'])
            every_week.add(Day=getnowtime('week'),Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            common.add(Ymd=getdaystime(1), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            common.add(Ymd=getdaystime(5), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
        elif cmd == '2':
            common.add(Ymd=getdaystime(30), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            every_month.delete('Con', my_dict_alt4['con'])
            every_year.add(MonthDay=getnowtime('md'),Con=my_dict_alt4['con'],  Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
    ...
    ############处理周的##############
        ...
        if cmd == '8':
            ...
            common.add(Ymd=getdaystime(10), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
            every_week.delete('Con', my_dict_alt4['con'])
            every_month.add(Day=getnowtime('d'), Con=my_dict_alt4['con'], Other1=my_dict_alt4['env'], Other2=my_dict_alt4['ext'])
